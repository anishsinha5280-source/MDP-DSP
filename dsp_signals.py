...
# ──────────────────────────────────────────────
# Advanced classes for Dashboard
# ──────────────────────────────────────────────

class SignalGenerator:
    def __init__(self, fs: float = 1000, duration: float = 1.0):
        self.fs = fs
        self.duration = duration
        self.t = np.arange(int(duration * fs)) / fs

    def sinusoid(self, freq: float, amp: float = 1.0):
        return amp * np.sin(2 * np.pi * freq * self.t)

    def noisy_sinusoid(self, freq: float, snr_db: float):
        _, clean, noisy = noisy_sinusoid(freq, snr_db, self.duration, self.fs)
        return noisy

    def get_all_demo_signals(self):
        return [
            {"name": "50Hz Sine", "data": self.sinusoid(50)},
            {"name": "Noisy 50Hz", "data": self.noisy_sinusoid(50, 10)},
            {"name": "ECG", "data": ecg_like(self.duration, self.fs)[1]}
        ]
...
class FilterBank:
    def __init__(self, fs: float):
        ...
        self.filters = [
            FilterWrap("Low-pass 100Hz", design_lowpass(16, 100, fs)),
            FilterWrap("Low-pass 200Hz", design_lowpass(16, 200, fs))
        ]

def snr_improvement(noisy, filtered, clean):
