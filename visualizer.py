...
def plot_overview(time, input_signal, filtered_signal, coefficients,
    ...
    """2-panel overview on one figure."""
    fig = plt.figure(figsize=(16, 5))
    fig.patch.set_facecolor(DARK_BG)
    fig.suptitle(f"Systolic Array FIR Filter — {scenario_name}",
                 color=TEXT_MAIN, fontsize=15, fontweight='bold', y=1.05)

    gs = gridspec.GridSpec(1, 2, wspace=0.32,
                           left=0.07, right=0.96, top=0.85, bottom=0.15)

    ax_sig = fig.add_subplot(gs[0, 0])
    ax_tap = fig.add_subplot(gs[0, 1])
    _apply_dark_style(fig, [ax_sig, ax_tap])
...
def plot_main_dashboard():
    ...
    coeffs = design_lowpass(16, 150, 1000)
    sa = SystolicArray(coeffs)
    filt = sa.filter(scenario["signal"])
    act = sa.get_activity_matrix()
    plot_overview(scenario["time"], scenario["signal"], filt, coeffs, act, 1000, 
                  scenario["name"], out_path="outputs/dashboard.png")

def animate_systolic_array(n_taps=8, n_steps=20, save_gif=True):
    """Placeholder for animation logic."""
    print("      - Generating Animation...")
    # Using the correct function name from animation.py
    from animation import build_animation as asa
    coeffs = np.ones(n_taps) * 0.1
    sa = SystolicArray(coeffs)
    _ = sa.filter(np.random.randn(n_steps))
    asa(sa.cycle_snapshots, coeffs, num_pe_display=n_taps, num_cycles_animate=n_steps, out_path="outputs/systolic_animation.gif")

def plot_filter_bank_response():
    """Wrapper for filter bank plot."""
    print("      - Generating Filter Bank Response...")
    from systolic_array import design_lowpass
    coeffs = design_lowpass(32, 200, 1000)
    plot_frequency_response(coeffs, 1000, title="Filter Bank Overview", out_path="outputs/filter_bank_response.png")

def plot_ecg_demo():
    """Wrapper for ECG demo plot."""
    print("      - Generating ECG Demo...")
    from dsp_signals import ecg_like
    t, sig = ecg_like(1.0, 500)
    fig, ax = plt.subplots(figsize=(10, 4))
    _apply_dark_style(fig, [ax])
    ax.plot(t, sig, color=ACCENT_CYAN)
    ax.set_title("ECG Signal Demo")
    _save(fig, "outputs/ecg_demo.png")
